...
import type { SubCategory } from "../types/salesMasters.types";
...
export default function SubCategoryList() {
  ...
  const { encSalesMasters, encSubCategory } = getEncryptedRoute();

  const ENC_NEW_PATH = `/${encSalesMasters}/${encSubCategory}/new`;
  const ENC_EDIT_PATH = (unique_id: string) =>
    `/${encSalesMasters}/${encSubCategory}/${unique_id}/edit`;

  const fetchSubCategories = async () => {
    try {
      const res = (await subCategoryApi.list()) as SubCategory[];
  ...
  const handleDelete = async (unique_id: string) => {
    ...
    await subCategoryApi.remove(unique_id);
