...
export default function DocumentTypeList() {
  ...
  const { encSalesMasters, encDocumentType } = getEncryptedRoute();

  const ENC_NEW_PATH = `/${encSalesMasters}/${encDocumentType}/new`;
  const ENC_EDIT_PATH = (unique_id: string) =>
    `/${encSalesMasters}/${encDocumentType}/${unique_id}/edit`;

  const fetchDocumentTypes = async () => {
    try {
      const res = await documentTypeApi.list();
  ...
  const handleDelete = async (unique_id: string) => {
    ...
    await documentTypeApi.remove(unique_id);
