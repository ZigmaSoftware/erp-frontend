...
import { itemCreationApi } from "@/helpers/admin";
import type { ItemCreation } from "../types/salesMasters.types";
...
export default function ItemCreationList() {
  ...
  const { encSalesMasters, encItemCreation } = getEncryptedRoute();

  const ENC_NEW_PATH = `/${encSalesMasters}/${encItemCreation}/new`;
  const ENC_EDIT_PATH = (unique_id: string) =>
    `/${encSalesMasters}/${encItemCreation}/${unique_id}/edit`;

  const fetchItems = async () => {
    try {
      const res = await itemCreationApi.list();
  ...
  const handleDelete = async (unique_id: string) => {
    ...
    await itemCreationApi.remove(unique_id);
  ...
  const statusTemplate = (row: ItemCreation) => {
    const updateStatus = async (value: boolean) => {
      await itemCreationApi.update(row.unique_id, {
