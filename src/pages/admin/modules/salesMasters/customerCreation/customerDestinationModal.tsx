...
import { customerDestinationApi } from "@/helpers/admin";
...
export default function CustomerDestinationModal({
  ...
  const loadRows = async (customerId: string) => {
    setLoading(true);
    try {
      const res = await customerDestinationApi.list({
  ...
  const handleDelete = async (row: CustomerDestination) => {
    ...
    try {
      await customerDestinationApi.remove(row.unique_id);
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (editingId) {
        await customerDestinationApi.update(editingId, payload);
      } else {
        await customerDestinationApi.create(payload);
      }
