...
import { rdfInertsPercEntryApi, siteApi } from "@/helpers/admin";
...
export default function RdfInertsPercEntryForm() {
  ...
  const { encSalesMasters, encRdfInertsPercEntry } = getEncryptedRoute();
  const ENC_LIST_PATH = `/${encSalesMasters}/${encRdfInertsPercEntry}`;
  ...
  useEffect(() => {
    ...
    const loadEntry = async () => {
      setFetching(true);
      try {
        const res = await rdfInertsPercEntryApi.get(id as string);
        const data = (res?.data || res) as RdfInertsPercEntry;
        setEntryNo(data.ri_perc_entry_no || "Auto Generated");
        setSiteName(normalizeRelationId(data.site_id || data.site_name));
        setRows(
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (isEdit) {
        await rdfInertsPercEntryApi.update(id as string, payload);
        Swal.fire({
          icon: "success",
        ...
        await rdfInertsPercEntryApi.create(payload);
        Swal.fire({
          icon: "success",
