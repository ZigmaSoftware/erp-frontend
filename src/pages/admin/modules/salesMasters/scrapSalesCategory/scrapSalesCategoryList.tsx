...
import { scrapSalesCategoryApi } from "@/helpers/admin";
import type { ScrapSalesCategory } from "../types/salesMasters.types";
...
export default function ScrapSalesCategoryList() {
  ...
  const { encSalesMasters, encScrapSalesCategory } = getEncryptedRoute();

  const ENC_NEW_PATH = `/${encSalesMasters}/${encScrapSalesCategory}/new`;
  const ENC_EDIT_PATH = (unique_id: string) =>
    `/${encSalesMasters}/${encScrapSalesCategory}/${unique_id}/edit`;

  const fetchCategories = async () => {
    try {
      const res = await scrapSalesCategoryApi.list();
  ...
  const handleDelete = async (unique_id: string) => {
    ...
    await scrapSalesCategoryApi.remove(unique_id);
  ...
  const statusTemplate = (row: ScrapSalesCategory) => {
    const updateStatus = async (value: boolean) => {
      await scrapSalesCategoryApi.update(row.unique_id, {
