...
export default function TargetEntryForm() {
  ...
  const ENC_LIST_PATH = `/${encSalesMasters}/${encTargetEntry}`;
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setFetching(true);
      try {
        const res = await targetEntryApi.get(id as string);
  ...
  const persistHeaderField = async (field: "site_id" | "entry_month", value: string) => {
    ...
    try {
      ...
      await targetEntryApi.update(targetEntryId, {
  ...
  const refreshItems = async (headerId: string) => {
    const res = await targetEntryItemApi.list({
  ...
  const createHeader = async (): Promise<TargetEntry> => {
    let candidate =
      targetNo || computeNextTargetNo((await targetEntryApi.list()) as TargetEntry[]);

    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        const headerRes = await targetEntryApi.create({
          target_no: candidate,
          site_id: siteId,
        ...
        if (hasTargetNoConflict && attempt === 0) {
          const latest = (await targetEntryApi.list()) as TargetEntry[];
  ...
  const handleAddRow = async () => {
    ...
    try {
      ...
      if (editingItemId) {
        await targetEntryItemApi.update(editingItemId, payload);
      } else {
        await targetEntryItemApi.create(payload);
      }
  ...
  const handleDeleteRow = async (item: TargetEntryItem) => {
    ...
    try {
      await targetEntryItemApi.remove(item.unique_id);
