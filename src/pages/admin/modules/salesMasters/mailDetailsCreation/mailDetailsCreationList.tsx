...
import { mailDetailsCreationApi } from "@/helpers/admin";
import type { MailDetailsCreation } from "../types/salesMasters.types";
...
export default function MailDetailsCreationList() {
  ...
  const { encSalesMasters, encMailDetailsCreation } = getEncryptedRoute();

  const ENC_NEW_PATH = `/${encSalesMasters}/${encMailDetailsCreation}/new`;
  const ENC_EDIT_PATH = (unique_id: string) =>
    `/${encSalesMasters}/${encMailDetailsCreation}/${unique_id}/edit`;

  const fetchMailDetails = async () => {
    try {
      const res = (await mailDetailsCreationApi.list()) as MailDetailsCreation[];
      setMailDetails(res);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (unique_id: string) => {
    ...
    await mailDetailsCreationApi.remove(unique_id);
  ...
  const statusTemplate = (row: MailDetailsCreation) => {
    const updateStatus = async (value: boolean) => {
      await mailDetailsCreationApi.update(row.unique_id, {
