...
import { mailDetailsCreationApi, siteApi } from "@/helpers/admin";
...
export default function MailDetailsCreationForm() {
  ...
  const ENC_LIST_PATH = `/${encSalesMasters}/${encMailDetailsCreation}`;
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setFetching(true);
      try {
        const res = await mailDetailsCreationApi.get(id as string);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (isEdit) {
        await mailDetailsCreationApi.update(id as string, payload);
        Swal.fire({
          icon: "success",
        ...
        await mailDetailsCreationApi.create(payload);
        Swal.fire({
          icon: "success",
