...
import { itemGroupCreationApi } from "@/helpers/admin";
import type { ItemGroupCreation } from "../types/salesMasters.types";
...
export default function ItemGroupCreationList() {
  ...
  const ENC_NEW_PATH = `/${encSalesMasters}/${encItemGroupCreation}/new`;
  const ENC_EDIT_PATH = (unique_id: string) =>
    `/${encSalesMasters}/${encItemGroupCreation}/${unique_id}/edit`;

  const fetchItemGroups = async () => {
    try {
      const res = (await itemGroupCreationApi.list()) as ItemGroupCreation[];
      setItemGroups(res);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (unique_id: string) => {
    ...
    await itemGroupCreationApi.remove(unique_id);
  ...
  const statusTemplate = (row: ItemGroupCreation) => {
    const updateStatus = async (value: boolean) => {
      await itemGroupCreationApi.update(row.unique_id, {
