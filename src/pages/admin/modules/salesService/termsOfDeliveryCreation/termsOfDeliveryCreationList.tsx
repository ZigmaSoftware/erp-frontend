...
import { termsOfDeliveryCreationServiceApi } from "@/helpers/admin";
import type { TermsOfDeliveryCreation } from "../types/salesService.types";
...
export default function TermsOfDeliveryCreationList() {
  ...
  const { encSalesService, encTermsOfDeliveryCreation } = getEncryptedRoute();

  const ENC_NEW_PATH = `/${encSalesService}/${encTermsOfDeliveryCreation}/new`;
  const ENC_EDIT_PATH = (unique_id: string) =>
    `/${encSalesService}/${encTermsOfDeliveryCreation}/${unique_id}/edit`;

  const fetchTermsOfDeliveries = async () => {
    try {
      const res = (await termsOfDeliveryCreationServiceApi.list()) as TermsOfDeliveryCreation[];
  ...
  const handleDelete = async (unique_id: string) => {
    ...
    await termsOfDeliveryCreationServiceApi.remove(unique_id);
  ...
  const statusTemplate = (row: TermsOfDeliveryCreation) => {
    const updateStatus = async (value: boolean) => {
      await termsOfDeliveryCreationServiceApi.update(row.unique_id, {
