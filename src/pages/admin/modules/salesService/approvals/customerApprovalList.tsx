...
import { customerCreationApi, approvalCustomerApi } from "@/helpers/admin";
...
export default function CustomerApprovalList() {
  ...
  const { encSalesService, encCustomerApproval } = getEncryptedRoute();

  const fetchCustomers = async () => {
    try {
      const res = (await customerCreationApi.list()) as CustomerCreation[];
      setCustomers(res);
    } finally {
      setLoading(false);
    }
  ...
  const handleAction = async (
    ...
    const remarks = await showRemarksDialog(`${label} Customer`);
    if (!remarks) return;

    try {
      await approvalCustomerApi.action(endpoint, { unique_id, remarks });
  ...
  return (
    <div className="px-3 py-3 w-full">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Review and approve pending customer creations
          </p>
        </div>
      </div>
      ...
      >
        ...
        <Column field="customer_name" header="Customer Name" sortable style={{ minWidth: "180px" }} />
        <Column field="contact_person" header="Contact Person" sortable style={{ minWidth: "150px" }} />
        <Column field="mobile_no" header="Mobile" sortable style={{ minWidth: "130px" }} />
        <Column
          header="Site"
          body={(row: CustomerCreation) => row.site_names?.join(", ") || row.sites?.join(", ") || "-"}
          style={{ minWidth: "140px" }}
        />
        <Column
          header="Customer Status"
          body={(row: CustomerCreation) => row.customer_status || "-"}
          style={{ width: "140px" }}
        />
        <Column header="Action" body={actionTemplate} style={{ width: "280px" }} />
      </DataTable>
    </div>
  );
}
