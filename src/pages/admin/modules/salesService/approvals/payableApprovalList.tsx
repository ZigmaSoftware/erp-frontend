...
import { payableApi, approvalPayableApi } from "@/helpers/admin";
...
export default function PayableApprovalList() {
  ...
  const { encSalesService, encPayableApproval } = getEncryptedRoute();
  ...
  const handleAction = async (
    ...
    const remarks = await showRemarksDialog(`${label} Payable`);
    if (!remarks) return;

    try {
      await approvalPayableApi.action(endpoint, { unique_id, remarks });
  ...
  return (
    <div className="px-3 py-3 w-full">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 mb-1">
            Payable Entry Approval
          </h1>
          <p className="text-gray-500 text-sm">
            Review and approve pending payable entries
          </p>
        </div>
      </div>
      ...
      <DataTable
        ...
        header={header}
        emptyMessage="No pending payables found."
        stripedRows
        showGridlines
        className="p-datatable-sm"
      >
        <Column header="S.No" body={indexTemplate} style={{ width: "80px" }} />
        <Column field="payable_no" header="Payable No" sortable style={{ minWidth: "150px" }} />
