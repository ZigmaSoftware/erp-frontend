...
import { invoiceApi, approvalInvoiceApi } from "@/helpers/admin";
import type { Invoice } from "../types/salesService.types";
...
export default function InvoiceApprovalList() {
  ...
  const { encSalesService, encInvoiceApproval } = getEncryptedRoute();

  const fetchInvoices = async () => {
    try {
      const res = (await invoiceApi.list()) as Invoice[];
      setInvoices(res);
    } finally {
      setLoading(false);
    }
  ...
  const handleAction = async (
    ...
    const remarks = await showRemarksDialog(`${label} Invoice`);
    if (!remarks) return;

    try {
      await approvalInvoiceApi.action(endpoint, { unique_id, remarks });
  ...
  const indexTemplate = (_: Invoice, { rowIndex }: { rowIndex: number }) =>
    rowIndex + 1;

  const actionTemplate = (row: Invoice) => (
  ...
  return (
    <div className="px-3 py-3 w-full">
      ...
      >
        ...
        <Column field="invoice_no" header="Invoice No" sortable style={{ minWidth: "150px" }} />
        ...
        <Column field="invoice_type" header="Invoice Type" sortable style={{ width: "130px" }} />
