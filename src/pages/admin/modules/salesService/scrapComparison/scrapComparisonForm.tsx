...
import { scrapComparisonApi, siteApi } from "@/helpers/admin";
import { getEncryptedRoute } from "@/utils/routeCache";
import { extractErrorMessage } from "@/utils/errorUtils";
import type {
  ScrapQuotationComparison,
  ScrapQuotationComparisonSub,
...
export default function ScrapComparisonForm() {
  const navigate = useNavigate();
  const { id } = useParams();
  const isEdit = Boolean(id);

  const { encSalesService, encScrapComparison } = getEncryptedRoute();
  const ENC_LIST_PATH = `/${encSalesService}/${encScrapComparison}`;
  ...
  /* ---------------- Sublist state ---------------- */
  const [items, setItems] = useState<ScrapQuotationComparisonSub[]>([]);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setFetching(true);
      try {
        const res = await scrapComparisonApi.get(id as string);
        const data = (res?.data || res) as ScrapQuotationComparison;
        ...
        Swal.fire({
          icon: "error",
          title: "Failed to load scrap comparison",
          text: "Something went wrong!",
        });
      } finally {
        setFetching(false);
  ...
  const persistHeaderField = async (field: string, value: string) => {
    ...
    try {
      ...
      await scrapComparisonApi.update(id, {
  ...
  const handleAddRow = async () => {
    ...
    try {
      ...
      if (!headerId) {
        const headerRes = await scrapComparisonApi.create({
          quote_month: `${quoteMonth}-01`,
          site_id: siteId,
          comp_description: description.trim(),
          status,
        });
        const headerData = (headerRes?.data || headerRes) as ScrapQuotationComparison;
        headerId = headerData.unique_id;
        navigate(`/${encSalesService}/${encScrapComparison}/${headerId}/edit`, { replace: true });
      }
      ...
      if (editingItemId) {
        await scrapComparisonApi.update(editingItemId, payload);
      } else {
        await scrapComparisonApi.create(payload);
      }

      if (headerId) {
        const res = await scrapComparisonApi.get(headerId);
        const data = (res?.data || res) as ScrapQuotationComparison;
        setItems(data.sub_items ?? []);
      }
  ...
  const handleEditRow = (item: ScrapQuotationComparisonSub) => {
  ...
  const handleDeleteRow = async (item: ScrapQuotationComparisonSub) => {
    ...
    try {
      await scrapComparisonApi.remove(item.unique_id);
      if (id) {
        const res = await scrapComparisonApi.get(id);
        const data = (res?.data || res) as ScrapQuotationComparison;
        setItems(data.sub_items ?? []);
      }
  ...
  return (
    <div className="p-8">
      <ComponentCard title={isEdit ? "Edit Scrap Comparison" : "Add New Scrap Comparison"}>
