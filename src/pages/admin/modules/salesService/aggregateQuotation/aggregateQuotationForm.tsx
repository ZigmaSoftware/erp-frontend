...
import { aggregateQuotationApi, siteApi } from "@/helpers/admin";
import { getEncryptedRoute } from "@/utils/routeCache";
import { extractErrorMessage } from "@/utils/errorUtils";
import type {
  AggregateQuotation,
  AggregateQuotationSub,
...
export default function AggregateQuotationForm() {
  const navigate = useNavigate();
  const { id } = useParams();
  const isEdit = Boolean(id);

  const { encSalesService, encAggregateQuotation } = getEncryptedRoute();
  const ENC_LIST_PATH = `/${encSalesService}/${encAggregateQuotation}`;
  ...
  /* ---------------- Sublist state ---------------- */
  const [items, setItems] = useState<AggregateQuotationSub[]>([]);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setFetching(true);
      try {
        const res = await aggregateQuotationApi.get(id as string);
        const data = (res?.data || res) as AggregateQuotation;
        ...
        Swal.fire({
          icon: "error",
          title: "Failed to load aggregate quotation",
          text: "Something went wrong!",
        });
      } finally {
        setFetching(false);
  ...
  const persistHeaderField = async (field: string, value: string) => {
    ...
    try {
      ...
      await aggregateQuotationApi.update(id, {
  ...
  const handleAddRow = async () => {
    ...
    try {
      ...
      if (!headerId) {
        const headerRes = await aggregateQuotationApi.create({
          party_name: partyName.trim(),
          party_mobile_no: partyMobileNo.trim(),
          site_id: siteId,
          quote_month: `${quoteMonth}-01`,
        });
        const headerData = (headerRes?.data || headerRes) as AggregateQuotation;
        headerId = headerData.unique_id;
        navigate(`/${encSalesService}/${encAggregateQuotation}/${headerId}/edit`, { replace: true });
      }
      ...
      if (editingItemId) {
        await aggregateQuotationApi.update(editingItemId, payload);
      } else {
        await aggregateQuotationApi.create(payload);
      }

      if (headerId) {
        const res = await aggregateQuotationApi.get(headerId);
        const data = (res?.data || res) as AggregateQuotation;
        setItems(data.sub_items ?? []);
      }
  ...
  const handleEditRow = (item: AggregateQuotationSub) => {
  ...
  const handleDeleteRow = async (item: AggregateQuotationSub) => {
    ...
    try {
      await aggregateQuotationApi.remove(item.unique_id);
      if (id) {
        const res = await aggregateQuotationApi.get(id);
        const data = (res?.data || res) as AggregateQuotation;
        setItems(data.sub_items ?? []);
      }
  ...
  return (
    <div className="p-8">
      <ComponentCard title={isEdit ? "Edit Aggregate Quotation" : "Add New Aggregate Quotation"}>
