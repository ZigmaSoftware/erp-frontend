...
import { itemGroupCreationServiceApi, itemTypeServiceApi } from "@/helpers/admin";
import { getEncryptedRoute } from "@/utils/routeCache";
import { extractErrorMessage } from "@/utils/errorUtils";
import { normalizeRelationId } from "@/utils/formHelpers";
import type { ItemGroupCreation, ItemType } from "../types/salesService.types";
...
export default function ItemGroupCreationForm() {
  ...
  const ENC_LIST_PATH = `/${encSalesService}/${encItemGroupCreation}`;
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setFetching(true);
      try {
        const res = await itemGroupCreationServiceApi.get(id as string);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (isEdit) {
        await itemGroupCreationServiceApi.update(id as string, payload);
        Swal.fire({
          icon: "success",
        ...
        await itemGroupCreationServiceApi.create(payload);
        Swal.fire({
          icon: "success",
