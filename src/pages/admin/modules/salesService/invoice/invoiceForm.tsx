...
import { invoiceApi, siteApi, customerCreationApi } from "@/helpers/admin";
import { getEncryptedRoute } from "@/utils/routeCache";
import type { Invoice, InvoiceSub } from "../types/salesService.types";
...
export default function InvoiceForm() {
  ...
  const { encSalesService, encInvoice } = getEncryptedRoute();
  const ENC_LIST_PATH = `/${encSalesService}/${encInvoice}`;
  ...
  /* ---------------- Sub-items ---------------- */
  const [items, setItems] = useState<InvoiceSub[]>([]);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setFetching(true);
      try {
        const res = await invoiceApi.get(id as string);
        const data = (res?.data || res) as Invoice;
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (isEdit) {
        await invoiceApi.update(id as string, payload);
        Swal.fire({
          icon: "success",
      ...
      const message =
        error?.response?.data?.invoice_no?.[0] ||
  ...
  const handleAddRow = async () => {
    ...
    try {
      ...
      if (editingItemId) {
        await invoiceApi.update(editingItemId, payload);
      } else {
        await invoiceApi.create(payload);
      }
  ...
  const handleEditRow = (item: InvoiceSub) => {
  ...
  const handleDeleteRow = async (item: InvoiceSub) => {
    ...
    try {
      await invoiceApi.remove(item.unique_id);
      if (id) {
        const res = await invoiceApi.get(id as string);
        const data = (res?.data || res) as Invoice;
        setItems(data.sub_items ?? []);
  ...
  return (
    <div className="p-8">
      <ComponentCard title={isEdit ? "Edit Invoice" : "Add New Invoice"}>
