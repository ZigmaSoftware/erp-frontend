...
import { workOrderApi } from "@/helpers/admin";
...
export default function WorkOrderList() {
  ...
  const ENC_NEW_PATH = `/${encSalesService}/${encWorkOrder}/new`;
  const ENC_EDIT_PATH = (unique_id: string) =>
    `/${encSalesService}/${encWorkOrder}/${unique_id}/edit`;
  ...
  const handleDelete = async (unique_id: string) => {
    ...
    await workOrderApi.remove(unique_id);
  ...
  return (
    <div className="px-3 py-3 w-full">
      ...
      >
        ...
        <Column
          header="DTC Status"
          body={(row: WorkOrder) => statusLabel(row.work_order_dtc_appr_status)}
          style={{ width: "120px" }}
        />
        <Column
          header="GM Status"
          body={(row: WorkOrder) => statusLabel(row.work_order_appr_status)}
          style={{ width: "120px" }}
        />
        <Column
          header="Director Status"
          body={(row: WorkOrder) => statusLabel(row.work_order_dt_appr_status)}
          style={{ width: "130px" }}
        />
