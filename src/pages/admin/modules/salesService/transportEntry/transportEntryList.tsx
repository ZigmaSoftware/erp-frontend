...
import { transportEntryServiceApi } from "@/helpers/admin";
...
export default function TransportEntryList() {
  ...
  const ENC_NEW_PATH = `/${encSalesService}/${encTransportMaster}/new`;
  const ENC_EDIT_PATH = (unique_id: string) =>
    `/${encSalesService}/${encTransportMaster}/${unique_id}/edit`;

  const fetchEntries = async () => {
    try {
      const res = await transportEntryServiceApi.list();
  ...
  const handleDelete = async (unique_id: string) => {
    ...
    await transportEntryServiceApi.remove(unique_id);
  ...
  const statusTemplate = (row: TransportEntry) => {
    const updateStatus = async (value: boolean) => {
      try {
        ...
        await transportEntryServiceApi.uploadUpdate(row.unique_id, formData);
        fetchEntries();
      } catch (error) {
        Swal.fire({
          icon: "error",
