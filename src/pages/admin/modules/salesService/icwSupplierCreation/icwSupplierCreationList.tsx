...
import { icwSupplierCreationServiceApi } from "@/helpers/admin";
import type { IcwSupplierCreation } from "../types/salesService.types";
...
export default function IcwSupplierCreationList() {
  ...
  const { encSalesService, encIcwSupplierCreation } = getEncryptedRoute();

  const ENC_NEW_PATH = `/${encSalesService}/${encIcwSupplierCreation}/new`;
  const ENC_EDIT_PATH = (uniqueId: string) =>
    `/${encSalesService}/${encIcwSupplierCreation}/${uniqueId}/edit`;

  const fetchSuppliers = async () => {
    try {
      const res = (await icwSupplierCreationServiceApi.list()) as IcwSupplierCreation[];
      setSuppliers(res);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (uniqueId: string) => {
    ...
    await icwSupplierCreationServiceApi.remove(uniqueId);
