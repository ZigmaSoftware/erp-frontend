...
export default function IcwSupplierCreationForm() {
  ...
  const ENC_LIST_PATH = `/${encSalesService}/${encIcwSupplierCreation}`;
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setFetching(true);
      try {
        const res = await icwSupplierCreationServiceApi.get(id as string);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (isEdit) {
        await icwSupplierCreationServiceApi.update(id as string, payload);
        Swal.fire({
          icon: "success",
        ...
        await icwSupplierCreationServiceApi.create(payload);
        Swal.fire({
          icon: "success",
